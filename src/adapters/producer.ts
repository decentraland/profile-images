...
export async function createProducerComponent({
  ...
  AppComponents,
  "awsConfig" | "config" | "logs" | "profileFetcher" | "storage"
>): Promise<JobProducer> {
  ...
  async function poll(ms: number, lastTimestamp: number): Promise<number> {
    const { profiles, timestamp } = await profileFetcher.getProfilesWithChanges(
      lastTimestamp
    );
    ...
    for (const [address, entity] of profiles) {
      ...
      logger.debug(
        `Added to queue address="${address}" and entity="${entity}"`
      );
    }
    return timestamp;
  }
